...
- **Input mappings promoted to sidebar** - Input mappings is now a top-level sidebar item directly between Consoles and Settings instead of being nested inside Settings, using the same controller icon as the panel title.
- **Settings Input section** - Settings now shows a bottom Input section with Player 1 and Player 2 input type dropdowns. Keyboard is available today; Gamepad and Virtual are visible but disabled placeholders for future support.
