# Game Icons attribution

The SVG files in this directory are sourced from
[game-icons.net](https://game-icons.net) and licensed under
[Creative Commons BY 3.0](https://creativecommons.org/licenses/by/3.0/).

| File | Author |
|---|---|
| `cassette.svg` | [Delapouite](https://delapouite.com) |
| `gamepad.svg` | [Delapouite](https://delapouite.com) |
| `joystick.svg` | [Delapouite](https://delapouite.com) |
| `retro-controller.svg` | [Skoll](https://game-icons.net/heads/skoll.html) |
| `save.svg` | [Delapouite](https://delapouite.com) |

The icons are processed at runtime to strip their negative-space
background and tint the foreground via `currentColor` — see
`../icons.ts` for the helper.
